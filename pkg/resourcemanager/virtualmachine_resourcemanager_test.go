...
func TestInitializeReportsVMFirmwareIdentity(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			firmware:   &kubevirtv1.Firmware{UUID: types.UID(firmwareUUID), Serial: firmwareSerial},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			vm := &kubevirtv1.VirtualMachine{
				ObjectMeta: metav1.ObjectMeta{Namespace: testNamespace, Name: testVMName, UID: types.UID(vmUID)},
