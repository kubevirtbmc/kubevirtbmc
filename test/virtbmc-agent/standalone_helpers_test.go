...
func TestStandaloneDeploymentUsesHealthzProbe(t *testing.T) {
	probe := standaloneDeployment("default").Spec.Template.Spec.Containers[0].ReadinessProbe
	if probe == nil || probe.HTTPGet == nil || probe.HTTPGet.Path != "/healthz" {
		t.Fatalf("standalone readiness probe must use the unauthenticated /healthz endpoint")
	}
}

func TestStandaloneRoleWatchesKubeVirtWithoutBMCPermissions(t *testing.T) {
	watched := map[string]bool{}
	for _, rule := range standaloneRole("default").Rules {
		for _, group := range rule.APIGroups {
			if group == bmcv1.GroupVersion.Group {
				t.Fatalf("standalone role must not require %s permissions", group)
			}
		}
		if !slices.Contains(rule.APIGroups, "kubevirt.io") || !slices.Contains(rule.Verbs, "watch") {
			continue
		}
		for _, resource := range rule.Resources {
			watched[resource] = true
		}
	}
	for _, resource := range []string{"virtualmachines", "virtualmachineinstances"} {
		if !watched[resource] {
			t.Errorf("standalone role must watch %s", resource)
		}
	}
}
...
// verifyStandaloneBootOverride checks the recorded boot override through
// Redfish: GET /Systems/1 renders BootSourceOverrideEnabled from the agent's
// boot state regardless of the VM's bootOrder fields. The IPMI readback
// (GetBootFlags) cannot serve here: it errors out when no device has a boot
// order — exactly the state after a cancel or reset — so it can never
// report "override absent".
func verifyStandaloneBootOverride(ctx context.Context, namespace string, shouldExist bool) {
	Eventually(func() bool {
		out, err := testutil.RunCurlRedfish(ctx, config, namespace, RedfishRequest{
		...
		disabled := strings.Contains(out, `"BootSourceOverrideEnabled":"Disabled"`)
		return disabled != shouldExist
	}, agentTestTimeout, agentTestInterval).Should(BeTrue(),
		"standalone boot override present should be %v (Redfish)", shouldExist)
}
